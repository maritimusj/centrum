...
func (g *Group) AddDevice(devices ...interface{}) error {
	err := g.store.TransactionDo(func(db helper.DB) interface{} {
		now := time.Now()
		...
		for _, device := range devices {
			...
			if exists, err := IsDataExists(db, TbDeviceGroups, "group_id=? AND device_id=?", g.id, deviceID); err != nil {
				return lang.InternalError(err)
			} else if exists {
				continue
			}
...
func (g *Group) RemoveDevice(devices ...interface{}) error {
	err := g.store.TransactionDo(func(db helper.DB) interface{} {
		for _, device := range devices {
			...
			if err := RemoveData(db, TbDeviceGroups, "group_id=? AND device_id=?", g.id, deviceID); err != nil {
...
func (g *Group) AddEquipment(equipments ...interface{}) error {
	err := g.store.TransactionDo(func(db helper.DB) interface{} {
		...
		for _, equipment := range equipments {
			...
			if exists, err := IsDataExists(db, TbEquipmentGroups, "group_id=? AND equipment_id=?", g.id, equipmentID); err != nil {
				return lang.InternalError(err)
			} else if exists {
				continue
			}
...
func (g *Group) RemoveEquipment(equipments ...interface{}) error {
	err := g.store.TransactionDo(func(db helper.DB) interface{} {
		for _, equipment := range equipments {
			...
			if err := RemoveData(db, TbDeviceGroups, "group_id=? AND equipment_id=?", g.id, equipmentID); err != nil {
				if err != sql.ErrNoRows {
					return lang.InternalError(err)
				}
			}
